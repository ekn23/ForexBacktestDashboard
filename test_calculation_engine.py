        ...
        for strategy_name, strategy_func in strategies.items():
            ...
            try:
                ...
                if trades:
                    ...
                    # Test win rate calculation with proper handling
                    if trades:
                        profitable_trades = sum(1 for trade in trades if trade.get('pnl', 0) > 0)
                        win_rate = (profitable_trades / len(trades)) * 100
                        profit_factor = abs(sum(t['pnl'] for t in trades if t['pnl'] > 0)) / abs(sum(t['pnl'] for t in trades if t['pnl'] < 0)) if sum(t['pnl'] for t in trades if t['pnl'] < 0) != 0 else 0
                        print(f"  - Win Rate: {win_rate:.2f}%")
                        print(f"  - Profit Factor: {profit_factor:.2f}")
                    else:
                        print("  - No trades to calculate metrics")

                    # Test max drawdown calculation with floating balance
                    if trades:
